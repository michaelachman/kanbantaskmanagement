  ...
  : 
    <div className="h-full w-full flex flex-row bg-red-500 pt-6 pl-4 text-left overflow-x-scroll flex-nowrap">
        {props.activeBoardStatusesArray?.map((status) => (
         ...
         <h1 className="column-title pb-6 text-lg">{status.statusName} ({props.activeBoardTasksArray?.filter((filteredTask) => filteredTask.statusId === status.id).length})</h1>
                {props.activeBoardTasksArray?.filter((filteredTask) => filteredTask.statusId === status.id).map((task) => (
              <div className="taskdiv px-4 py-6 mb-5 bg-green-500 rounded-lg shadow-xl">
              <h2 className="bg-blue-500">{task.statusId === status.id && task.taskTitle}</h2>
              <h3>{props.subtasksArray?.filter((filteredSubtask) => filteredSubtask.taskId === task.id && filteredSubtask.subtaskStatus === true).length} of {props.subtasksArray?.filter((filteredSubtask) => filteredSubtask.taskId === task.id).length}</h3>
